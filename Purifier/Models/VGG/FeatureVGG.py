...
class Conv_FC_Block(tnn.Module):
    ...
    def __init__(self, chann_in, chann_out, k_size, p_size,i):
        ...
        if config['Global']['dataset'] == 'GTSRB':
            self.fc = tnn.Linear(4*self.channel_out, 43)
        else:
            self.fc = tnn.Linear(4*self.channel_out, 10)
            
    def forward(self, x, label=None):
        out = self.conv(x)
        out = self.bn(out)
        out = self.relu(out)
        fc_in = out.view(out.shape[0], -1)
...
def vgg_conv_block_channel_reg(in_list, out_list, k_list, p_list, pooling_k, pooling_s):
    layers = [Conv_FC_Block(in_list[i], out_list[i], k_list[i], p_list[i], i) for i in range(len(in_list))]
    # layers += [tnn.MaxPool2d(kernel_size=pooling_k, stride=pooling_s)]
...
class VGG16_feature(tnn.Module):
...
def feature_vgg16(**kwargs):
    return VGG16_feature(**kwargs)